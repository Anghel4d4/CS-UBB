...
class REPOSITORY:
    ...
    def load_from_file(self):
        ...
        with open(self.__filename, 'r') as f:
            ...
            for line in lines:
                if line.strip():
                    try:
                        nume, prenume, inaltime, post = line.strip().split('/')
                        jucator = Jucator(nume, prenume, inaltime, post)
                        lista.append(jucator)
                    except ValueError:
                        print(f"Linia nu are formatul corect: {line.strip()}")
        return lista
    ...
    def save_to_file(self, lista):
        ...
        with open(self.__filename, 'w') as f:
            for el in lista:
                string = el.get_nume() + '/' + el.get_prenume() + '/' + str(
                    el.get_inaltime()) + '/' + el.get_post() + '\n'
                f.write(string)
